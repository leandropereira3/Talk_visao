...
var msgTipoSessaoNova = 'NV';
var msgTipoSessaoAntiga = 'AN';

function init(){
	usuarioLogado = sessionStorage.getItem("login");	
	connect();
	setRefreshingContatosAceitos();
	//setRefreshingContatosCadastrados();
}
...
function addConversaEnvio(texto){
	...
	if(msgData){
		...
		var div = '<div style="height: 50px; width: 55%"><div style="font-size: 16px;">'+msgData[0]+'</div><div style="font-size: 11px;">'+msgData[1]+'</div></div>'
...
function setRefreshingContatosCadastrados(){
	console.log('refreshing contatos cadastrados...');
	$.ajax({
		type : "GET",			
		url : urlRest + 'usuarioService/getUsuarios/',
		success : function(dados) {						
			$('#novocontato').empty().append('<option value=""></option>');

			for (var i = 0; i < dados.length; i++) {
				var userId = dados[i].id;
				var userName = dados[i].apelido;
				var option = '<option value="'+userId+'">'+userName+'</option>';
				$('#novocontato').append(option);
			}			
...
function selectContato(loginContato){	
	...
	$.ajax({
		...
		success : function(dados) {
			for (var i = 0; i < dados.length; i++) {
				var msg = dados[i];				
				addConversaHistorico('Recebido: ' + msg);
			}
		}, //END success
		error : function(e) {				
			alert('Erro: Não foi possível obter os dados do Web Service.');
